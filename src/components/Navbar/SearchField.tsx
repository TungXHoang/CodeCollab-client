...
const SearchField = ({ projectsList }: { projectsList: IProjectList }) => {
	...
	return (
		<form onSubmit={handleKeyDown } className="flex relative w-64 mx-4 font-normal">
			...
			{searchField && showSearchDropdown &&
				...
				<ul className="dropdown-shadow absolute top-[32px] right-0 left-0 z-10 text-[hsl(0,0%,80%)] py-1 bg-[hsl(220,10%,20%)]">
				{
						...
						<>
							{[...filteredProject.owner, ...filteredProject.guest].map((project) => (
								<li className="text-[12px]" key={project._id}>
									<a onMouseDown={(e) => e.preventDefault()} className="border-b-[1px] border-[#3c3c3c] block p-2 !pl-3 hover:bg-[hsl(220,9%,25%)] hover:text-[hsl(0,0%,100%)]" href={`${import.meta.env.VITE_CLIENT_BASEURL}/edit/${project._id}`}>
